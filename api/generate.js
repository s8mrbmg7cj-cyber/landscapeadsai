...
<div class="page" id="page-builder">
  <div class="builder-wrap">
    <div class="builder-panel">
      ...
      <div class="fg"><label class="fl">CTA Button Text</label><input class="fi" id="bCta" placeholder="e.g. Call Now for Free Estimate"></div>
...
async function genAndRender() {
  ...
  try {
    const d = await apiCall({ type:'visual', biz, city, service, offer, phone, template: curTmpl });
    // ctaText is the canonical field; cta is the legacy fallback
    const headline      = d.headline       || `Professional ${service}`;
    const supportingText= d.supportingText || 'Licensed & Insured • Free Estimates';
    const ctaText       = d.ctaText        || d.cta || phone;
    // Write AI results into the editable fields on the left panel
    ...
    // Store full ad data for canvas render
    curAd = {
    ...
    // Populate fallback values so canvas can still render
    const fallbackHeadline = document.getElementById('bHeadline').value || `Professional ${service}`;
    const fallbackSubtext  = document.getElementById('bSubtext').value  || 'Licensed & Insured • Free Estimates';
    const fallbackCta      = document.getElementById('bCta').value      || phone;
...
async function genVariations() {
  ...
  try {
    const d = await apiCall({ type:'variations', biz, city, service, offer, phone });
...
async function genCaption() {
  ...
  try {
    const d = await apiCall({ type:'caption', biz, city, service, offer, phone });
...
async function genContent30() {
  ...
  try {
    const d = await apiCall({ type:'content30', biz, city, service, offer });
    const weeks = d.weeks || [];
...
async function runGenerate(){
  ...
  try{
    const d=await apiCall({type:'generate',biz,city,service,offer,target,tone,platforms});
